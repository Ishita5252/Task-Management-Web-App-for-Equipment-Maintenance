...
function CreateMaintenanceMachineComponent(){
    ...
    const [machineLocation, setMachineLocation] = useState("DTA");
    ...
    const updateScheduleAsync = () => {
        return new Promise((resolve, reject) => {
            if(!taskFrequency || !taskFrequency2 || !frequencyUnit || !dueFromDate){
                resolve({
                    dueFromDate: null,
                    dueTillDate: null
                });
            ...
            if(doneOnDate && dueFromDate <= doneOnDate){
    ...
    const saveOrUpdateMachine = async (e) => {
        ...
        if(id === '_add'){
            ...
            navigate('/ground-floor/maintenanceMachines');
        } else {
