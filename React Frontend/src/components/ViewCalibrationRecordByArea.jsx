...
function ViewCalibrationRecordByArea(){
    ...
    return (
        <div>
            ...
            <h6 className="text-center">Monthly Preventive Calibration Schedule</h6>
            <h6 className="text-center">({area})</h6>
            <h6 className="text-end">{currMonth}'{currYear}</h6>
                    ...
                    <tbody>
                        {
                            machines.map((machine, index) => {
                                ...
                                let formattedLastDoneDate = ('0' + lastDoneDate.getUTCDate()).slice(-2) + '-' + ('0' + (lastDoneDate.getUTCMonth()+1)).slice(-2)
                                 + '-' + lastDoneDate.getUTCFullYear().toString().substring(2);

                                let nextDueDate = new Date(machine.nextDueDate);
                                let formattedNextDueDate = ('0' + nextDueDate.getUTCDate()).slice(-2) + '-' + ('0' + (nextDueDate.getUTCMonth()+1)).slice(-2)
