...
import ExcelGeneratorCalInstance from '../services/ExcelGeneratorCal';
...
function ListCalibrationMAcineComonent(){
    ...
    const fetchMachines = useCallback(() => {
        CalibrationMachineService.getMachinesByFloor(floor).then((res) => {
    ...
    const editMachine = (id) => {
        navigate(`/root/calibrationMachine/add-calibration-machine/${id}/${floor}`);
    }

    const addMachine = () => {
        navigate(`/root/calibrationMachine/add-calibration-machine/_add/${floor}`);
    }

    const generateScheduleExcel = (floor) => {
        ExcelGeneratorCalInstance.generateExcelMaintenance(floor);
    }


    const viewRecordByArea = (area) => {
        generateScheduleExcel(floor);
        navigate(`/root/calibrationMachine/view-record/${area}`)
    }
    ...
    return(
        <div>
            <br></br>
            <h2 className="text-center">Equipment List for Calibration - {displayed_floor}</h2>
            <br></br>
