    ...
    return (
        <div>
            ...
            <div className="text-end">
                <h6>Month: {currMonth}'{currYear}</h6>
                    ...
                    <tbody>
                        {
                            machines.map((machine, index) => {
                                ...
                                let formattedDueFromDate = ('0' + dueFromDate.getUTCDate()).slice(-2) + '/' + ('0' + (dueFromDate.getUTCMonth()+1)).slice(-2)
                                 + '/' + dueFromDate.getUTCFullYear().toString().substring(2);

                                let dueTillDate = new Date(machine.dueTillDate);
                                let formattedDueTillDate = ('0' + dueTillDate.getUTCDate()).slice(-2) + '/' + ('0' + (dueTillDate.getUTCMonth()+1)).slice(-2)
                                ...
                                return(
                                    <tr key={machine.id} class='default-height'>
                                        ...
                                        <td className="table-data" style={{ fontSize: '0.6rem' }}> {formattedDueFromDate} - {formattedDueTillDate}</td>
